...
def test_process_intent_is_destroy():
    response = client.post("/process", json={"input": "tear down the staging environment"})
    assert response.status_code == 200
    assert response.json()["intent"] == "DESTROY"


def test_process_intent_is_query():
    response = client.post("/process", json={"input": "what is the current state of the production cluster"})
    assert response.status_code == 200
...
def test_process_extracts_explicit_infra():
    response = client.post("/process", json={"input": "provision an EC2 instance with RDS"})
    assert response.status_code == 200
    kit = response.json()
...
def test_process_extracts_exclusion():
    response = client.post("/process", json={"input": "deploy a web app on AWS, no kubernetes"})
    assert response.status_code == 200
    kit = response.json()
